...
from kraken.plugins.consumers.keavem.model.exceptions import DecodingUndefinedItemCount
from kraken.plugins.consumers.keavem.model.structure import (
    MeasData,
    MeasFileHeader,
    MeasInfo,
    MeasValue,
...
class StrMetaCP4Codec(Type[Union[str, datetime]]):
    ...
    def decode_raw(data: bytes, slc: slice) -> Union[str, datetime]:
        ...
        if 18 >= len(chunk) > 14:
            return datetime.strptime(chunk, "%Y%m%d%H%M%SZ")
        return chunk
